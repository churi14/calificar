...
export default function FunnelClient({ business, employee }: { business: Business; employee: Employee }) {
  ...
  async function handleStarClick(star: number) {
    ...
    if (star >= threshold + 1) {
      ...
      // Redirigir a Google con pequeño delay para que vean la selección
      setTimeout(() => {
        setStage('positive')
        setTimeout(() => {
    ...
    try {
      if (business.negative_redirect === 'whatsapp' && business.whatsapp_number) {
        ...
        await fetch('/api/feedback', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ business_id: business.id, rating: selected, message, contact })
        })
        setStage('feedback-sent')
      }
    } finally {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="px-8 pt-8 pb-6 text-center"
            style={{ background: `linear-gradient(135deg, ${business.primary_color}15, ${accent}15)` }}>
            {business.logo_url ? (
            ...
            {employee && (
              <p className="text-sm text-gray-500 mt-1">con {employee.name}</p>
            )}
          </div>
          ...
          {stage === 'rating' && (
            <div className="px-8 pb-8 pt-4">
              <p className="text-center text-gray-700 font-semibold text-base mb-2">
                ...
                {[1, 2, 3, 4, 5].map(star => (
                  <button key={star}
                    ...
                    <svg width="44" height="44" viewBox="0 0 24 24"
                      ...
                      strokeWidth="1.5"
                      style={{ filter: displayStars >= star ? `drop-shadow(0 0 8px ${accent}80)` : 'none',
              ...
              <p className="text-center text-sm font-medium text-gray-500 h-5">
                ...
                {displayStars === 4 && 'Muy bueno'}
                {displayStars === 5 && '¡Excelente!'}
              </p>
            </div>
          )}

          {/* Stage: POSITIVE → redirigiendo a Google */}
          {stage === 'positive' && (
          ...
          {/* Stage: NEGATIVE → feedback privado */}
          {stage === 'negative' && (
            <div className="px-8 pb-8 pt-4">
              ...
              <input
                ...
                className="w-full mt-3 rounded-xl border border-gray-200 p-3 text-sm text-gray-700 focus:outline-none focus:border-gray-400 placeholder-gray-300"
              />

              <button
                onClick={sendFeedback}
                disabled={!message.trim() || sending}
                className="w-full mt-4 py-3 rounded-xl text-white font-semibold text-sm disabled:opacity-50 transition-opacity"
                style={{ background: business.primary_color }}>
